...
def get_financial_details(entities,docs,text,nlp):
    entity = []
    for ent in entities:
        match = re.search(r"(Rs\.?|₹)\s?\d+[,\d]*(?:\.\d+)?\s*(?:crores?|cr|lakhs?|lakh|million|bn)?", ent, re.IGNORECASE)
        match = re.search(r"(Rs\.?|₹)\s?\d+[,\d]*(?:\.\d+)?\s*(?:crores?|cr|lakhs?|lakh|million|bn)?", ent, re.IGNORECASE)
        if match:
...
def ner_extraction(text, nlp):
    ...
    # Regex backup for missed cases
    regex_dates = re.findall(r"\d{2}[-/]\d{2}[-/]\d{4}(?: \d{2}:\d{2}(?::\d{2})?)?", text)
    regex_money = re.findall(r"(?:Rs\.?|₹)\s?\d+[,\d]*(?:\.\d+)?\s*(?:crores?|cr|lakhs?|lakh)?", text, re.IGNORECASE)

    # Merge unique
    all_dates = set(spacy_dates + regex_dates)
